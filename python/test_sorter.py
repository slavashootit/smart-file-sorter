...
def setup_sandbox(sandbox_path):
    ...
    # Створюємо унікальні файли в корені
    ...
    # Створюємо дублікати в корені (однаковий вміст, різні назви)
    ...
    # Створюємо чисті та змішані підпапки
    pure_videos = sandbox_path / "pure_videos"
    pure_videos.mkdir()
    (pure_videos / "clip1.mp4").write_text("clip 1 video")
    (pure_videos / "clip2.mov").write_text("clip 2 video")
    
    pure_pics = sandbox_path / "pure_pics"
    pure_pics.mkdir()
    (pure_pics / "photo1.jpg").write_text("photo 1")
    (pure_pics / "photo2.png").write_text("photo 2")
    
    mixed_stuff = sandbox_path / "mixed_stuff"
    mixed_stuff.mkdir()
    (mixed_stuff / "pic.jpg").write_text("pic in mixed")
    (mixed_stuff / "contract.pdf").write_text("contract in mixed")
    
    empty_folder = sandbox_path / "empty_folder"
    empty_folder.mkdir()
...
def test_full_cycle():
    ...
    # Перевіримо, що файли та папки не перемістилися насправді
    assert (sandbox / "video1.mp4").exists()
    assert (sandbox / "image_copy.png").exists()
    assert (sandbox / "pure_videos").exists()
    assert (sandbox / "pure_pics").exists()
    assert (sandbox / "mixed_stuff").exists()
    assert (sandbox / "empty_folder").exists()
    ...
    # Перевіряємо результати сортування файлів
    ...
    # Перевіряємо результати сортування підпапок
    assert (sandbox / "Відео" / "pure_videos").exists(), "Чиста папка 'pure_videos' має бути перенесена у 'Відео'"
    assert (sandbox / "Відео" / "pure_videos" / "clip1.mp4").exists()
    assert (sandbox / "Зображення" / "pure_pics").exists(), "Чиста папка 'pure_pics' має бути перенесена у 'Зображення'"
    assert (sandbox / "Зображення" / "pure_pics" / "photo1.jpg").exists()
    
    # Змішана та порожня папки мають залишитися на місці
    assert (sandbox / "mixed_stuff").exists(), "Змішана папка повинна залишитися на місці"
    assert (sandbox / "mixed_stuff" / "pic.jpg").exists()
    assert (sandbox / "mixed_stuff" / "contract.pdf").exists()
    assert (sandbox / "empty_folder").exists(), "Порожня папка повинна залишитися на місці"
    ...
    assert len(history_data["moves"]) == 8, f"Має бути записано 8 успішних переміщень (6 файлів + 2 папки), але записано {len(history_data['moves'])}"
    ...
    # Перевіряємо, чи всі файли та папки повернулися назад
    ...
    assert (sandbox / "pure_videos").exists(), "Папка 'pure_videos' мала повернутися назад"
    assert (sandbox / "pure_videos" / "clip1.mp4").exists()
    assert (sandbox / "pure_pics").exists(), "Папка 'pure_pics' мала повернутися назад"
    assert (sandbox / "pure_pics" / "photo1.jpg").exists()
    assert (sandbox / "mixed_stuff").exists()
    assert (sandbox / "empty_folder").exists()
    ...
    assert not (sandbox / "Документи").exists(), "Папка 'Документи' має бути видалена після скасування"
    ...
    print("✓ Скасування (Undo) працює бездоганно (всі файли, папки та підпапки відновлено)")
